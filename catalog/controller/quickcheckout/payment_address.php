...
class ControllerQuickCheckoutPaymentAddress extends Controller {
	public function index() {
		...
		$inline_var = array('lastname', 'address_1');
		$ignore_var = array('address_id', 'zone_id', 'zone_code', 'country_id', 'iso_code_2', 'iso_code_3', 'custom_field');
		foreach($data['addresses'] as &$address){
			$address_formated = array();
			foreach($address as $var => $value){
				if(in_array($var, $ignore_var)) continue;
				if(trim($value)){
					if(in_array($var, $inline_var)){
						$address_formated[count($address_formated) - 1] = ' ' . $value;
					}
					else{
						$address_formated[] = $value;
					}
				}
			}
			$address['address_formated']	=	implode(', ', $address_formated);
		}
		...
		if (!$json) {
			...
			if ($this->request->post['payment_address'] == 'new') {
				...
				if (!$json) {
					...
					// especially for customer that using social login
